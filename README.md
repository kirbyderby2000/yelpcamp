...
2. Create a user account with that email address (Google OAuth is fine as long as the Gmail account is the email address)
3. Visit the administrative dashboard link from the site navbar.


## 🌱 Seeding Instructions
There is a seeding feature to seed the database with campgrounds to play around with. Note that seed images cannot be deleted from Cloudinary because it would make break reseeding the application on future seeds.

Here's how to setup the seed feature:
1. Configure the seed config in `appConfig.js` under `seedConfig`
   - `seedUserEmailAddress` : This is the email address that will linked as the user account of the seed data. The account created with this email address will be injected the `seedUserId` as its ID, forcing an associated link between the seed data and the provided user.
